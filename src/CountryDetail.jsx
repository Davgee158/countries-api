    ...
    const fetchCountry = async () => {
      try {
        ...
        const codeToName = Object.fromEntries(
          data.map((c) => [c.alpha3Code, c.name])
        );
      ...
      >
        <p className="text-lg font-bold mb-4">
          {error === "Country not found"
            ? "Country not found"
            : "Oops! Couldn't load country"}
        </p>
        <button
          onClick={() => navigate("/")}
          className={`px-6 py-2 rounded shadow-md mr-2
  ...
  return (
    <main className="max-w-[375px] md:max-w-[700px] md:mx-auto xl:max-w-[1100px] 2xl:max-w-[1440px] py-12 px-6">
      <button
        ...
        className={`${
          isDarkMode
            ? "bg-dark-blue-text transition-colors duration-300 ease-in out"
            : "bg-white"
        } 
  py-2 px-8 md:text-base md:px-10 text-xs shadow-md rounded hover:cursor-pointer 
  transition-colors transition-transform duration-200 ease-out hover:scale-[1.05]
  focus:outline-none focus:ring-2`}
      >
        Back
      </button>
      <div className="xl:flex gap-20 2xl:gap-32 mt-12">
        ...
        <div className="py-4">
          <h1 className="font-extrabold text-xl md:text-3xl pb-3">
            {country.name}
          </h1>
          ...
          <div>
            <h2 className="pr-4 mt-6 font-semibold md:text-lg">
              Border Countries:{" "}
            </h2>
            <div className="mt-2 md:mt-4 flex flex-wrap gap-2">
              {(country?.borders || []).map((code) => {
                ...
                return (
                  <span
                    key={code}
                    className={`px-5 py-1 md:px-8 md:py-2 mr-2 md:mr-3 text-sm md:text-base rounded-xs md:rounded-sm shadow-sm whitespace-nowrap transition-colors duration-300 ease-in out ${
                      isDarkMode ? "bg-dark-blue-bg " : "bg-white"
                    } ${
                      isDarkMode ? "text-gray-300" : "text-gray-800"
